// Program to add N numbers using mpi reduce
...
int main(int argc, char** argv) {
    ...
    MPI_Reduce(&local_sum, &final_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);  // reducing their sum down to a single number
    if(my_rank == 0) {
        for(int i = 0; i < MAX_SIZE%world_size; i++)
            final_sum += vector[MAX_SIZE - 1 - i]; // calculating sum of the remaining numbers
    }
